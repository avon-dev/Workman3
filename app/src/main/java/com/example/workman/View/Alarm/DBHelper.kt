...
class DBHelper(val context: Context) : SQLiteOpenHelper(context,DBHelper.DATABASE_NAME,null,DBHelper.DATABASE_VERSION) {
    ...
    private val COL_MELODY_NAME = "melody_name"
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val createTable = "CREATE TABLE $TABLE_NAME ($COL_ID INTEGER, $COL_HOUR  INTEGER,$COL_MINUTE  INTEGER,$COL_DAYS  STRING,$COL_MELODY STRING,$COL_MELODY_NAME STRING, $COL_ON_OFF INTEGER)"
    ...
    fun addAlarm(alarm: Alarm): Boolean{
        ...
        contentValues.put(COL_MELODY_NAME, alarm.melody_name)
        ...
        if(result.moveToFirst()){
            do {
                val alarm = Alarm(
                    ...
                    result.getString(result.getColumnIndex(COL_MELODY)),
                    result.getString(result.getColumnIndex(COL_MELODY_NAME))
                )
